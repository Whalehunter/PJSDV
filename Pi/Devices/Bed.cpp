...
bool Bed::updateStatus()
{
    ...
    /* check if client is still connected through receive, if not -> close socket and end the thread */
...
{
    /* lock the mutex to protect critical data from being mutated by multiple threads at the same time */
